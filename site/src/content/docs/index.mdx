...
Every provider here is certified by *passing the same suite* — no exceptions, no self-attestation. Each clears the upstream authoritative baseline **plus** an extension suite, against **92 conformance behaviors across 11 areas**, credential-free on every change. That covers the things you'd otherwise have to take on faith: a failed bootstrap or drain surfaces as a visible failure rather than a silent false "ready"; launches are idempotent; inventory stays consistent.
