---
title: Observability
description: The libvirt provider's metrics catalogue, health vs readiness, the logging model, and a sample Prometheus scrape.
sidebar:
  order: 6
  label: Observability
---

The libvirt provider is built to be operated from its signals. Every libvirt API
...
The libvirt client is wrapped by a transparent metrics decorator, so every API
