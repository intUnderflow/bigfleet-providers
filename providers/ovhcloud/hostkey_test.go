...
// Two generations must produce distinct keys + fingerprints (no fixed/shared key).
func TestGenerateHostKey_Unique(t *testing.T) {
	...
	if a.fingerprint == b.fingerprint || a.privatePEM == b.privatePEM {
		t.Error("two generated host keys are identical; want unique per server")
	}
}
...
func TestHostKeyCallback_Verify(t *testing.T) {
	...
	// A second, different key against a non-empty pin is still rejected (the pin
	// is what's verified, not the first key seen).
	other := testPubKey(t)
	if err := hostKeyCallback(fp, nil)("h", nil, other); err == nil {
		t.Error("a different key matched the pin; want rejection")
	}
}
