...
//   - Droplet side: a small agent, installed by the GENERIC pre-binding user_data
...
// Droplet to fetch its own blob via a per-machine bearer token. The token is
...
// client injects it into the Droplet's generic user_data at Create.
