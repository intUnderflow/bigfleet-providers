...
// labelHostKeyFP pins a server's SSH host-key fingerprint so Configure/Drain can
// verify the host they connect to. The value is base32(sha256(host pubkey)) — 52
// lowercase [a-z2-7] chars, well within UpCloud's label-value rules.
const labelHostKeyFP = "bigfleet-host-key-fp"

// machineIDEncoding encodes the machine id (and other tokens) into a
// label/title-safe value (lowercase base32 without padding → only [a-z2-7]).
...
// a key we already know; the fingerprint is pinned in a label and verified on
// every later SSH connection.
type hostKeyMaterial struct {
	...
	fingerprint string // base32(sha256(pubkey wire bytes)) — pinned in labelHostKeyFP
}
...
// hostKeyFingerprint is the stable, label-safe pin for an SSH host public key:
// base32(sha256(wire bytes)), lowercased. Deterministic across processes, so a
// fingerprint persisted in a label round-trips a provider restart.
