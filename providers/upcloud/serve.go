...
// recovery + request-logging/metrics interceptor chain, the standard gRPC health
// service, and (optionally) reflection for debugging.
func buildGRPCServer(creds credentials.TransportCredentials, m *metrics, reflect bool, logger *slog.Logger) (*grpc.Server, *health.Server) {
	opts := []grpc.ServerOption{
		...
		// Logging is the OUTER interceptor and recovery the inner one, so a panic in
		// the handler is recovered into codes.Internal and then still recorded by the
		// logging interceptor (rpcCalls{code=Internal} + rpcDuration), not swallowed
